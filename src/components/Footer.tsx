...
type BrandingData = { logo?: string; logoMark?: string; brandName?: string; tagline?: string; brandColor?: string; phone?: string; whatsapp?: string; email?: string; address?: string; footerText?: string };
...
export default function Footer() {
  ...
  useEffect(() => {
    ...
    try {
      const s = localStorage.getItem("ff_branding");
    ...
    fetch("/api/admin/branding")
      ...
      .then((data) => {
        if (data.ok && data.data) {
          setB(data.data);
          localStorage.setItem("ff_branding", JSON.stringify(data.data));
        }
      })
      .catch(() => {});
  }, []);
    ...
    <footer className="bg-slate-900 text-white">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12">
          <div className="lg:col-span-2">
            <Link href="/" className="flex items-center gap-2 mb-6">
              {b.logoMark ? (
                <img src={b.logoMark} alt="" className="w-10 h-10 rounded-xl object-cover" />
              ) : (
                <div className="w-10 h-10 rounded-xl text-white flex items-center justify-center font-bold text-lg" style={{ backgroundColor: b.brandColor || "#2563eb" }}>{(b.brandName || "F").charAt(0)}</div>
              )}
              <div>
                <h2 className="text-lg font-bold">{b.brandName || "Founders & Footsteps"}</h2>
                <p className="text-xs text-white/60">{b.tagline || "Apex Lifestyle Syndicate"}</p>
              </div>
            </Link>
            ...
            <div className="space-y-3 text-sm">
              <div className="flex items-start gap-3 text-white/70"><Phone className="w-4 h-4 mt-0.5 flex-shrink-0" /> <span>{b.phone || "0261404904"}</span></div>
              <div className="flex items-start gap-3 text-white/70"><MessageCircle className="w-4 h-4 mt-0.5 flex-shrink-0" /> <span>WhatsApp: {b.whatsapp || "0257664762"}</span></div>
              <div className="flex items-start gap-3 text-white/70"><Mail className="w-4 h-4 mt-0.5 flex-shrink-0" /> <span>{b.email || "phrimpongkelvin@gmail.com"}</span></div>
              <div className="flex items-start gap-3 text-white/70"><MapPin className="w-4 h-4 mt-0.5 flex-shrink-0" /> <span>{b.address || "Accra, Ghana"}</span></div>
            </div>
          </div>
          <div>
      ...
      <div className="border-t border-white/10">
        <div className="container mx-auto px-4 py-6 flex flex-col md:flex-row items-center justify-between gap-4">
          <p className="text-white/50 text-sm">{b.footerText || `© ${new Date().getFullYear()} Founders & Footsteps · Apex Lifestyle Syndicate. All rights reserved.`}</p>
