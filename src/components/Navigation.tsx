...
import { Menu, X, ShoppingCart, ChevronDown, LogOut, LayoutDashboard, Home, Car, Utensils, Plane, Scissors, Truck, Wrench, ShoppingBag, Inbox, Palette } from "lucide-react";
...
const serviceLinks = [
  ...
  { name: "Graphic Design", slug: "/services/graphic-design", icon: Palette, color: "#f97316" },
  { name: "Marketplace", slug: "/marketplace", icon: ShoppingBag, color: "#ef4444" },
];

type BrandingData = { logo?: string; logoMark?: string; brandName?: string; tagline?: string; brandColor?: string };
...
export default function Navigation() {
  ...
  // Load branding from server then localStorage
  useEffect(() => {
    // First try localStorage for instant display
    try {
      const stored = localStorage.getItem("ff_branding");
        ...
        <div className="container mx-auto px-4">
          <div className="flex items-center justify-between h-16">
            <Link href="/" className="flex items-center gap-2 flex-shrink-0">
              {branding.logoMark ? (
                <img src={branding.logoMark} alt="Logo" className="w-10 h-10 rounded-xl object-cover" />
              ) : (
                <div className="w-10 h-10 rounded-xl text-white flex items-center justify-center font-bold text-lg" style={{ backgroundColor: branding.brandColor || "#2563eb" }}>{(branding.brandName || "F").charAt(0)}</div>
              )}
              <div className="hidden sm:block">
                {branding.logo ? (
                  <img src={branding.logo} alt={branding.brandName || "Logo"} className={`h-8 object-contain ${isScrolled ? "" : "brightness-0 invert"}`} />
                ) : (
                  <>
                    <h1 className={`text-lg font-bold leading-tight ${isScrolled ? "text-slate-900" : "text-white"}`}>{branding.brandName || "Founders & Footsteps"}</h1>
                    <p className={`text-[10px] uppercase tracking-widest ${isScrolled ? "text-slate-500" : "text-white/70"}`}>{branding.tagline || "Apex Lifestyle Syndicate"}</p>
                  </>
                )}
              </div>
            </Link>
