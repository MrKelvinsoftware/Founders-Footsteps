...
export default function CarRentalPage() {
  const { user } = useAuth();
  const cars = useMemo(() => mergeWithSeed(seedCars, "cars" as any) as CarType[], []);
  ...
  const [bookedCar, setBookedCar] = useState<CarType | null>(null);
                ...
                <div className="p-6">
                  ...
                  <div className="flex items-end justify-between pt-4 border-t border-slate-100">
                    <div>
                      ...
                      <p className={`text-xs mt-1 font-medium ${(car.stock ?? 5) > 0 ? "text-emerald-600" : "text-red-600"}`}>
                        {(car.stock ?? 5) > 0 ? `${car.stock ?? 5} available` : "Out of stock"}
                      </p>
                    </div>
                    <button 
                      onClick={() => { if ((car.stock ?? 5) > 0) { setBookedCar(car); setSelected(car.id); } }} 
                      disabled={(car.stock ?? 5) === 0}
                      className={`px-4 py-2 rounded-full text-white text-sm font-semibold ${(car.stock ?? 5) === 0 ? "bg-slate-300 cursor-not-allowed" : "bg-blue-600 hover:bg-blue-700"}`}
                    >
                      {(car.stock ?? 5) === 0 ? "Unavailable" : "Book Now"}
                    </button>
                  </div>
                </div>
              </div>
