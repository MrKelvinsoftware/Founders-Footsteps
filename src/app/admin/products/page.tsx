...
export default function AdminProductsPage() {
  ...
  const saveToServer = async (newCustom: Product[], newHidden: string[]) => {
    setSaving(true);
    try {
      await fetch("/api/cms/products", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ products: newCustom, hidden: newHidden }),
      });
    } catch (e) {
  ...
  // Merge custom products with seed products
  const all = useMemo(() => {
  ...
  const save = async () => {
    ...
    let newCustom: Product[];
    const existingIdx = custom.findIndex((c) => c.id === payload.id);
    
    if (existingIdx >= 0) {
      newCustom = [...custom];
      newCustom[existingIdx] = payload;
    } else {
      newCustom = [payload, ...custom];
    }

    setCustom(newCustom);
    await saveToServer(newCustom, hidden);
    setDrawerOpen(false);
    flash("Saved and synced to live catalogue!");
  };

  const onDelete = async (p: Product) => {
    if (!confirm(`Delete "${p.name}"? This cannot be undone.`)) return;
    const newCustom = custom.filter((c) => c.id !== p.id);
    setCustom(newCustom);
    await saveToServer(newCustom, hidden);
    flash("Deleted.");
  };
  ...
  const onHide = async (p: Product) => {
    ...
    if (hidden.includes(p.id)) {
      newHidden = hidden.filter((x) => x !== p.id);
    } else {
      newHidden = [...hidden, p.id];
    }
    setHidden(newHidden);
    await saveToServer(custom, newHidden);
  };
      ...
      <header className="sticky top-0 z-30 bg-white border-b border-slate-200">
        <div className="flex items-center justify-between px-6 py-4">
          ...
          <div className="flex items-center gap-2">
            {saving && <RefreshCw className="w-4 h-4 text-orange-500 animate-spin" />}
        ...
        <div className="fixed inset-0 z-50 flex justify-end">
          ...
          <div className="relative w-full max-w-xl bg-white h-full overflow-y-auto shadow-2xl animate-[slideInRight_.3s_ease]">
            ...
            <div className="sticky bottom-0 bg-white border-t border-slate-200 px-6 py-4 flex gap-3">
              ...
              <button onClick={save} disabled={saving} className="flex-1 px-4 py-3 rounded-full bg-slate-900 text-white font-semibold hover:bg-orange-500 flex items-center justify-center gap-2 disabled:opacity-50">
                {saving ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Save className="w-4 h-4" />} Save & publish
              </button>
            </div>
          </div>
        </div>
