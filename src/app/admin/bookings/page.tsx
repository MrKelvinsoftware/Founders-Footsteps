...
import {
  ...
  Home, Utensils, Scissors, Plane, CalendarCheck, ChevronRight, Truck, Wrench, Car, Palette,
} from "lucide-react";
...
const typeMeta: Record<SubmissionType, { label: string; icon: React.ComponentType<{ className?: string }>; color: string; accent: string }> = {
  ...
  "graphic-design": { label: "Graphic Design", icon: Palette, color: "#f97316", accent: "bg-orange-50 text-orange-700" },
};
...
export default function AdminBookingsPage() {
  ...
  const counts = useMemo(() => ({
    ...
    "graphic-design": bookings.filter((b) => b.type === "graphic-design").length,
  }), [bookings]);
